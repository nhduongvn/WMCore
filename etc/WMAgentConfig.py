...
config.AgentStatusWatcher.pendingSlotsSitePercent = 50  # [percent] Pending slots percent over site max running for a site
config.AgentStatusWatcher.pendingSlotsTaskPercent = 45  # [percent] Pending slots percent over task max running for tasks
...
config.RucioInjector.blockRuleParams = {"copies": 2}
# blockDiskRuleRSEExpr: broader expression used for non-T0 block rules (copies=2).
# Rucio keeps the existing origin-T2 replica as copy-1 and replicates a second copy elsewhere.
