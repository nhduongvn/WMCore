...
class RucioInjectorPoller(BaseWorkerThread):
    ...
    def insertBlockRules(self):
        ...
        expression with copies=2 so Rucio keeps one copy at the origin site (where the
        data already exists) and replicates a second copy to another RSE for redundancy.
        For T0 agents (or when blockDiskRuleRSEExpr is not set), pins to the specific
        origin T2 RSE with copies=1 (legacy behaviour).
